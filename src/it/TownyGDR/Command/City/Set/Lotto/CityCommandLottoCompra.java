 ...
 * 
 *********************************************************************/
public class CityCommandLottoCompra extends CommandManager {

	private static ArrayList<String> perm = new ArrayList<String>();
	static{
		perm.add("City.lotto.compra");
	}
	
	/**
	 * @param perm
	 */
	public CityCommandLottoCompra() {
		super(perm);
	}
		...
		if(sender instanceof Player) {
			...
			if(this.checkPermission(p)) {
				...
				if(pd.getCity() != null) {
					...
					//compra il lotto
					boolean res = mem.getLotto().compraLotto(city.getArea().LottoVenditagetByElementoArea(ele), mem);
					
					//feedBack
					if(res) {
						p.sendMessage("Lotto comprato!");
					}else{
						p.sendMessage(ChatColor.RED + "Lotto NON comprato!");
					}
				}else{
